  ...
  renderImg = ({src, alt}) =>{
    return (
      <div className="wrap_thumb">
        <img src={src} className="img_thumb" alt={alt}/>
      </div>
    )
  }
  ...
  renderItem = ({id, name, title, desc, imgSrc, imgInfo, shadow, link}, index)=>{
    return(
      <li key={id}>
        {
          link && link !== '' 
            ? <a href="javascript:;" className="">{this.renderInfo({imgSrc, title, name, desc})}</a>
            : <div className="">{this.renderInfo({imgSrc, title, name, desc})}</div>
        }
      </li>
    )
  }
    ...
    return(
      <div className="comp_news_content">
        ...
        <div className="comp_content_box">
          <ul>
            {DATA.newsData.map(this.renderItem)}
          </ul>
        </div>
      </div>
    )
